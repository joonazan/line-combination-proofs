...
Programming languages that completely eliminate invalid memory accesses or even the bulk of crashes have entered the mainstream. Yet all mainstream languages are Turing-complete and as a consequence allow writing programs that keep running forever. (Infinite loops are undefined behaviour in C, so arguably programs containing them are not valid and recognizing valid C programs is undecidable.) Writing programs that not only run to completition but even behave as intended is still commonly viewed as infeasible or prohibitively expensive.

At the same time, computers have become an integral part of mathematical research. This is rather surprising – while mathematicians have published many mistakes, most programs contain several, in some cases several per line of code. Still, computer programs are good for testing conjectures on millions of different inputs in the hope of finding a small counterexample that is easy to check by hand. But not all outputs are easy to check.
