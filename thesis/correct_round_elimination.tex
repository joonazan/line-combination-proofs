...
\end{minted}
relating weight to strict domination. With well-foundedness proved, we can use the \icoq{well_founded_induction} from the standard library in \icoq{combination_is_complete}.
...
Dealing with permutations seemed like something that could benefit from existing theories, so I switched to the Mathematical components library at that point. It was originally created as part of a formal proof of the Four color theorems. It contains theorems about abstract algebra and common data structures.

Mathematical components is built on the ssreflect library, which has a strong opinion on how Coq proofs should be written and provides utilities for writing in that particular style. I have adopted this style in my proof.

An important idea in ssreflect is that every proposition is written as a boolean expression and converted into the most suitable form via a reflection view when necessary.~\cite{mathcompbook} This is good because it makes you think about how you want to proceed with the proof instead of just blindly destructing every conjunction and splitting every disjunction. However, often that is precisely what needs to be done and in that case ssreflect only creates more work.
