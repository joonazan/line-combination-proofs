...
The idea of using proof assistants to verify algorithms that are used in proofs isn't new.
...
\begin{proof}
  ...
  We can translate between $((a, b), c)$ and $(a, (b, c))$, so $\times$ is associative. Incidentally in Coq $((1, 2), 3)$ is the same as $(1, 2, 3)$ but $(1, (2, 3))$ has a different type. The neutral element of $\times$ is $1$, as it doesn't add any information, like struct padding in C.

  Distributivity: $A \times (B + C) = (A \times B) + (A \times C)$. It is clear that either type could be used with only surface-level changes to program code. As discussed earlier, $0$ is impossible to construct, so a tuple containing it is, too: $A \times 0 = 0$.
\end{proof}

All types with the same number of inhabitants are isomorphic, so instead of the long proof one could just have stated that $|A + B| = |A| + |B|$ and $|A \times B| = |A||B|$. With that in mind it is easy to see that the arrow $\to$ in function types corresponds to exponentiation; a function is just a table of outputs with an entry for each input. $|A -> B| = |B|^{|A|}$.

Inductive types form interesting equations, for example
\begin{align*}
...
\end{align*}
Conor McBride~\cite{typeDerivative} has even shown that partial derivatives of types can be useful.

\subsubsection{Dependent functions}

pi types are arrays of values
