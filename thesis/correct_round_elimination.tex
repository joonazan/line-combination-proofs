...
I have formalized the proof presented in Section~\ref{nlproof} and an implementation of maximization in Coq. I've written a proof of that implementation's correctness.
...
\subsubsection{Line combination}
...
\subsection{Performance on the LCL classifier dataset}

Both my and Olivetti's implementation of the new algorithm compares new lines against other lines originating from the same two lines before comparing against every line. This is worthwhile based on the assumption that many of the new lines are just inferior versions of the best lines of that batch.

Olivetti showed me that maintaining a set of dominated lines and checking new lines against that set before doing any comparisons improves performance by an order of magnitude in practice, which means that at least in the inputs tested, some useless lines are created very many times.
