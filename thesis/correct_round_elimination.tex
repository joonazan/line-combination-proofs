...
The idea of using proof assistants to verify algorithms used as a part of a proof isn't new.

The most notable example is Thomas Hales' proof of the Kepler conjecture. A proof consisting of a computer program that solves linear programming problems and hundreds of pages of notes was completed in 1998. After four years of inspecting the proof, a panel of twelve referees announced that they couldn't be completely certain if the computer calculations are correct. This prompted Hales to start the Flyspeck project, which completed a complete formal proof of the Kepler conjecture in 2014.
...
In this section I'll show that the round elimination produces a problem that can be solved exactly one round faster than its predecessor in the port numbering model.
...
\begin{proof}
  The $r$-neighborhood is sufficient, as information from further away cannot be seen in $r$ rounds. The $r$-neighborhood can be constructed in $r$ rounds, so it isn't too much information either. TODO how exactly
\end{proof}
...
When I started this project, I chose Coq simply because it is very mature and I knew it was capable of this kind of task. I now know that type theory based proof assistants like Coq are especially suited to this kind of task because they are capable of \emph{reflection}.
...
To verify a program in a proof assistant that does not support programming, one has to generate lemmas from the source code that is being verified. This is how the seL4 microkernel~\cite{sel4} has been developed, for example. It makes sense in that context because high performance and low level access to hardware is vital when implementing an operating system.
