...
A combination $C$ of two lines $A$ and $B$ only generates configurations present in $A$ or $B$.

Proof: For each configuration $c$ generated by $C$, wlog.\ suppose the symbol chosen from the union is from $A$. The symbols that come from intersections are in both $A$ and $B$. Thus all symbols in $c$ are from $A$.
...
For any non-singleton line $l$ there exist two lines strictly dominated by $l$ that can be combined into $l$.

Proof:

Any line can be built by combining singleton lines.
...
A line dominating any valid line can be built by combining input lines.

Proof:

Corollary: The maximal lines can be built this way.

Proof: A line is maximal if it isn't strictly dominated by any valid line, thus a valid line dominating a maximal line must be equal to it. Lines obtained by combining input lines are valid (TODO reference earlier lemma).

\subsection{Efficiently finding the maximal lines}

But even if combining lines eventually produces the maximal lines, that doesn't mean it is an efficient way of finding them.

Define a \emph{missing line} as a valid line that isn't dominated by any of the input lines.

If there is a missing line, then some missing line can be obtained by combining two input lines. In other words, we can make progress by simply trying all combinations of two lines!

Proof:

This gives an efficient process for checking maximality: if all ways of combining two lines result in lines dominated by some existing line, then the current set of lines is maximal.
