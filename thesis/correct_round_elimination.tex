...
There are proof systems that simply check the rules of a logic.

Coq is more complicated than that. It is a full programming language whose type checking can be interpreted as proof verification. This complexity makes it unsuitable for ATPs as-is but it enables \emph{reflection}. (cite this? http://adam.chlipala.net/cpdt/html/Reflection.html)

Suppose we need to show that problem $\Pi_1$ is one round faster than $\Pi_0$. Instead of showing that directly, which would be tedious, we can instead run an implementation of round elimination written in Coq on $\Pi_0$ to obtain $\Pi_1$ and then invoke the proof that that implementation works as intended.

Simpler arithmetic example? Expand and sort polynomial, then compare for equality. Write proof that that is the same as equality.
...
In the port numbering model, each vertex runs the same program and must produce its own part of the solution. Each vertex has numbered ports through which it communicates with its neighbors.

Computation work as follows: first, vertices get to compute as much as they need to. Then they send a message to each of their neighbors. All vertices receive the messages simultaneously. This process repeats until all vertices have chosen their output.
...
Before discussing the new round elimination algorithm, I need to explain the basics of Coq because I will show the lemmas concerning its correctness as written in Coq's Vernacular. I will also give informal proofs. The actual proofs involve many boring details, which this thesis does not cover. However, the Proving section will show examples of the most important proof techniques used. The full source code can be found in the project's repository~\cite{source_code}.
...
In this section I will present a maximization algorithm based on combining lines and prove its correctness. In a later section, I will present the corresponding Coq lemmas. Section \ref{proving} contains information on how the mechanized proofs were developed.
...
In this section I will present a maximization algorithm based on combining lines and the lemmas that I used to prove its correctness. I will show the most important lemmas as they are written in Coq but present the proofs informally. The Proving section contains information on how the mechanized proofs were developed.
...
If every configuration was allowed, the maximal form could be expressed as a line that generates every configuration. The old algorithm starts with a list containing just that line. It then goes through all the invalid configurations, replacing every line in its list with narrowed down lines that do not generate that configuration.
...
The new algorithm's runtime does not depend on the size of the alphabet.
