...
In Coq, the type of a simple type is Set, the type of set is Type0, the type of Type0 is Type1 and the type of that is Type2 etc. Types have subtyping: anything of a lower type may be used as a higher type.~\cite{CPDT}

The universe hierarchy also forces predicativity, meaning that a construct can't take itself as an argument. For example an identity function taking itself as an argument is perfectly benign but impredicativity tends to cause inconsistency in logical systems. Coquand~\cite{newParadox} reports on one such paradox he has discovered.
