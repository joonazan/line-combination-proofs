...
Round Elimination is an algorithm that transforms an RE problem. That transformation has interpretations in many models of distributed computing.
...
Brandt et al.~\cite{speedup} introduced round elimination in 2019. As the name suggests, it takes a problem $\Pi_0$ that can be solved in $r$ rounds and outputs a new problem $\Pi_1$ that is solvable in $r-1$ rounds. But what is $\Pi_1$ relationship to $\Pi_0$?
